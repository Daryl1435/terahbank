...
import { ArrowLeftRight, Users, Shield, Download } from 'lucide-react';
...
export default function ReportsPage() {
  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        <h1 className="font-poppins font-semibold text-2xl text-navy">{t('reports.title')}</h1>
        <p className="text-mid-grey text-sm mt-1">{t('reports.subtitle')}</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-card p-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h2 className="font-poppins font-semibold text-lg text-navy">{t('reports.transactions.title')}</h2>
              <p className="text-mid-grey text-sm">{t('reports.transactions.subtitle')}</p>
            </div>
          </div>
          ...
          <div className="space-y-3 mb-5">
            <div>
              <label className="block text-xs font-medium text-dark-grey mb-1">
                {t('reports.transactions.dateStart')}
              </label>
              <input
                type="date"
                value={dateFrom}
            ...
            <div>
              <label className="block text-xs font-medium text-dark-grey mb-1">
                {t('reports.transactions.dateEnd')}
              </label>
              <input
                type="date"
                value={dateTo}
          ...
          >
            {loading === 'transactions' ? <>{spinner} {t('reports.preparing')}</> : <><Download className="w-4 h-4" /> {t('reports.download')}</>}
          </button>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-card p-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h2 className="font-poppins font-semibold text-lg text-navy">{t('reports.users.title')}</h2>
              <p className="text-mid-grey text-sm">{t('reports.users.subtitle')}</p>
            </div>
          </div>

          <div className="bg-off-white rounded-lg p-4 mb-5 text-sm text-dark-grey">
            <p className="font-medium mb-1">{t('reports.users.columns')}:</p>
          ...
          >
            {loading === 'users' ? <>{spinner} {t('reports.preparing')}</> : <><Download className="w-4 h-4" /> {t('reports.download')}</>}
          </button>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-card p-6">
          <div className="flex items-center gap-3 mb-4">
            <div className="w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0" style={{ backgroundColor: 'rgba(11,138,138,0.1)' }}>
              <Shield className="w-5 h-5" style={{ color: '#0B8A8A' }} />
            </div>
            <div>
              <h2 className="font-poppins font-semibold text-lg text-navy">{t('reports.insurance.title')}</h2>
              <p className="text-mid-grey text-sm">{t('reports.insurance.subtitle')}</p>
            </div>
          </div>

          <div className="bg-off-white rounded-lg p-4 mb-5 text-sm text-dark-grey">
            <p className="font-medium mb-1">{t('reports.insurance.columns')}:</p>
            <p className="text-xs text-mid-grey font-mono">
          ...
          >
            {loading === 'insurance' ? <>{spinner} {t('reports.preparing')}</> : <><Download className="w-4 h-4" /> {t('reports.download')}</>}
          </button>
        </div>
      </div>
